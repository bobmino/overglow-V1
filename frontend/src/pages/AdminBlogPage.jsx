...
const AdminBlogPage = () => {
  ...
  const getStatusBadge = (isPublished) => {
    if (isPublished) {
      return (
        <span className="px-3 py-1 rounded-full text-xs font-bold bg-primary-100 text-primary-800 flex items-center gap-1">
          <CheckCircle size={12} />
    ...
    <div className="space-y-6">
      ...
      {posts.length === 0 ? (
        <div className="bg-gray-50 rounded-xl p-12 text-center">
          ...
          <Link
            to="/admin/blog/new"
            className="px-6 py-3 bg-primary-600 text-white rounded-lg font-semibold hover:bg-primary-700 transition inline-flex items-center gap-2"
          >
            <Plus size={18} />
            ...
            {posts.map((post) => (
              <div key={post._id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition">
                ...
                <div className="p-6">
                  ...
                  {post.isPublished && (
                    <div className="text-xs text-primary-700 bg-primary-50 border border-primary-200 rounded-lg px-3 py-2 mb-4">
